...
export const VERIFICATION_INSTRUCTIONS = dedent`
  ...
  - If verifyIdentity reports the identity check failed, tell the caller the information did not match and ask them to say the last four digits again, slowly. The tool allows three attempts total, so do not give up before they are used. When the tool reports attempts are exhausted, it records the outcome; tell the caller you cannot discuss the account today, suggest they call back with correct information, and call end_call.
